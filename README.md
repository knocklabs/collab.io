...
1. Setup .env file and variables:
   First run this command to create a new `.env` file.
...
The `KNOCK_SECRET_API_KEY` is required to seed Knock resources as a part of the following setup process.

The `NEXT_PUBLIC_KNOCK_PUBLIC_API_KEY` is required for the in-app feed and guides.

The `KNOCK_SIGNING_KEY` is required for signing a JWT needed for the Slack integration.

Learn more about the different types of [API keys in Knock](https://docs.knock.app/developer-tools/api-keys).
...
If you want to view or inspect the database, your can use Prisma studio:
...
As a part of bootstrapping the database, the following resources are created for you. You can use these account emails and passwords to sign into the application as different characters.
